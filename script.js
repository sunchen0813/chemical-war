...
function compoundactive(){
    ...
    if(complish==true){
        ...
        else if(product.mode==0)document.getElementById("product").innerHTML='<button type="button" id="productbutton" class="elementbutton" onclick="productdelete_element('+product.mode+')"> <img src="picture/trash.png" id="productimage" class="elementimage"></button>';
        else{
...
function ohminus(){
    if(compound[0].ion.OHminus==true){//非沉澱物
        ...
        if(compound[0].ion.transition==true){
            if(compound[0].ion.twonature==true)product.mode=8;
            else product.mode=12;
       }else{
            if(compound[0].ion.twonature==true)product.mode=6;
            else product.mode=11;
       }
    }
    product.product=null;
    product.element1=compound[0].ion;
...
function form(){
    ...
    document.getElementById("endgame1").innerHTML=`<p id="endgame"></p><p id="endgamecontent">${result}</p><a href="index.html"><input id="endgamebutton" type="button" value="回首頁"></input></a><div class="container">
    <form action="" method="post" name="google" id="form">
    <label for="fname">暱稱</label>
    <input style="font-size:25px;color:blue;" type="text" id="fname" name="firstname" placeholder="暱稱..">

    <label for="country">身分</label>
    <select id="country" name="country">
        <option style="font-size:25px;color:blue;" value="student">學生</option>
        <option style="font-size:25px;color:blue;" value="teacher">老師</option>
    </select>

    <label for="learn">是否學過沉澱表</label>
    <select id="learn" name="learn">
        <option style="font-size:25px;color:blue;" value="Yes">Yes</option>
        <option style="font-size:25px;color:blue;" value="No">No</option>
    </select>

    <label for="subject">建議</label>
  ...
  <input name="H+produce" value="${ele[8].produce}"></input>
  <input name="H+pick" value="${ele[8].pick}"></input>
  <input name="H+compound" value="${ele[8].compound}"></input>
  <input name="OH-produce" value="${ele[28].produce}"></input>
  <input name="OH-pick" value="${ele[28].pick}"></input>
  <input name="OH-compound" value="${ele[28].compound}"></input>
...
/*     const scriptURL2='https://script.google.com/macros/s/AKfycbyRdNJLYyBKKa2tmvK27BWYJUwSbZCgFmvn7OhXg0m4Wk_YdGUIIL0fi6B8uK6qkt-y/exec'
    ...
    const form = document.forms['google'];
      ...
      form.addEventListener('submit', e => {
        ...
        fetch(scriptURL, { method: 'POST', body: new FormData(form) })
          .then(response => alert("感謝遊玩，祝每次段考都喔趴！！！"))
...
function getValues(){
    ...
    attack0:attack[0].compound,
    attack1:attack[1].compound,
    attack2:attack[2].compound,
    attack3:attack[3].compound,
    attack4:attack[4].compound,
    attack5:attack[5].compound,
    attack6:attack[6].compound,
    attack7:attack[7].compound,
    attack8:attack[8].compound,
    attack9:attack[9].compound,
    attack10:attack[10].compound,
    attack11:attack[11].compound,
    towerblood:towerblood
...
function openhint(){
    window.open ("page.html", "newwindow", "height=700, width=600, top=0, left=500, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no")
}
...
function startCountDown(duration, element) {
    ...
    let countInterval = setInterval(function (c) {
        ...
        else if(secondsRemaining==0&&end==2){
            end=0;
            form();
        } 
        if(minerspace==9)minersec=0;
        ...
        if(fightsec%30==15&&enemyspace<5)produceenemy();
...
var end=2;//1獲勝0時間到-1合成錯誤太多
var trashnumber=0;
let secondsRemaining = 0;
window.onload = function (c) {
    let time_minutes = 0; // Value in minutes
    let time_seconds = 1; // Value in seconds
